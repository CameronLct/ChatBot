...
class ActionShowReservationInfo(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        hour = tracker.get_slot("hour_reservation")
        place = tracker.get_slot("number_place_reservation")
        dispatcher.utter_message(text=f"Information de réservation :"
                                      f"\nPrénom : {first_name}"
                                      f"\nPlace: {place}"
                                      f"\nHeure de réservation : {hour}"
                                      f"\n Commentaire : {comment}")
        return []